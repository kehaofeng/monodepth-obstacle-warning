...
### 1. 序列分布（柱状图）
26 个序列的样本数差异较大：最大为 `drive_0009`（443 帧）和 `drive_0039`（352 帧），最小仅 29~49 帧。整体以短序列为主，说明 KITTI 采集覆盖了不同时长的驾驶场景。划分时按序列整体分配，避免了同一场景下的相似帧泄露到多个集合。
...
### 3. 图像尺寸（分布统计）
存在两种分辨率：`375×1242`（2011_09_26，约 43%）和 `370×1224`（2011_09_28，约 57%）。这是不同采集批次使用的相机型号差异所致。训练时需统一缩放到固定分辨率（如 640×192）。
